...
import com.ksbm.ontu.foundation.time.live_clock.Hand;
import com.ksbm.ontu.foundation.time.live_clock.HourHand;
import com.ksbm.ontu.foundation.time.live_clock.MinuteHand;

import java.util.Calendar;

import static com.ksbm.ontu.utils.Constant.hourHandStatus;
import static com.ksbm.ontu.utils.Constant.minuteHandStatus;
...
public class AnalogClock1 extends View {
    ...
    public AnalogClock1(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        ...
        // Log.e("hr_mint", ""+ hour + " "+ minute);

        Paint hourHandPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        hourHandPaint.setColor(getResources().getColor(R.color.dark_goldenrod));
        ...
        minuteHandPaint.setColor(getResources().getColor(R.color.chocolate));
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        hourTickPaint.setColor(getResources().getColor(R.color.brown));
    ...
    private boolean onHandMoveStart(MotionEvent event) {
        double dMinute, dHour;
        Hand hand=null;
        double distance = 0;

        if (minuteHandStatus) {
             dMinute = minuteHand.getDistanceTo(event.getX(), event.getY());
            hand = minuteHand;
            distance = dMinute;
        }

        if (hourHandStatus) {
             dHour = hourHand.getDistanceTo(event.getX(), event.getY());
            hand = hourHand;
            distance = dHour;
        }
